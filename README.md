...
NetScope uses **PLT Hook** (Procedure Linkage Table hooking) via [shadowhook](https://github.com/bytedance/android-inline-hook) to intercept libc network calls (`connect`, `send`, `recv`, `getaddrinfo`, `close`, etc.) inside the app process. It attributes traffic to domain names via:
...
- **Java TLS via Conscrypt**: OkHttp and Android's `HttpsURLConnection` route TLS through Conscrypt (BoringSSL via JNI), bypassing `libc.so` `send`/`recv`. The `connect`/`close` hooks still fire, so connections are tracked, but byte counts for Java HTTPS traffic will be 0. C++ NDK network calls go through `libc.so` directly and are fully captured.
